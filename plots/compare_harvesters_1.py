...
from harvester_emission_splits import compare_harvester_yield
...
def plot_atlas_harvest_comparison_1(atlas_parts=0):
    ...
    _x_parts = np.linspace(0, 500, 501) # 1 to 500 parts
    _x_legions = np.linspace(0, 2000, 2001) # 1 to 2000 legions


    for parts, legions in zip(_x_parts, _x_legions):
        ...
        (
            ...
            mine_pct_shares,
            user_pct_shares,
        ) = compare_harvester_yield(harvesters=harvesters, debug=True)
